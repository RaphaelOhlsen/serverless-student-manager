...
Os Environments `dev-bootstrap-admin` e `dev-resume-first-admin-invitation` exigem reviewer, impedem bypass administrativo e vinculam os jobs a trust policies OIDC com `sub` exato, sem wildcards. Os workflows usam credenciais temporárias OIDC e não armazenam credenciais AWS estáticas. Suas Environment variables estão completas e verificadas, sem Environment secrets. As duas capacidades estão tecnicamente prontas para `workflow_dispatch`, mas nenhuma execução operacional ou validação end-to-end foi realizada.
