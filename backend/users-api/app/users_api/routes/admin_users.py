...
class RoleChangeServiceProtocol(Protocol):
    def change_role(
        self,
        *,
        cognito_sub: str,
...
def register_admin_user_routes(
    ...
    @app.post("/users/<user_id>/role-change")
    def change_user_role(
        ...
        try:
            ...
            active_service = (
                role_change_service
                if role_change_service is not None
                else get_role_change_service()
            )
            result = active_service.change_role(
                cognito_sub=cognito_sub,
                user_id=user_id,
                idempotency_key=key,
                request_id=request_id,
            ...
            return _error_response(400, "INVALID_REQUEST", "Invalid role change request")
        except AdminUserUnauthorizedError:
            return _error_response(401, "UNAUTHORIZED", "Unauthorized")
        except AdminUserForbiddenError:
            return _error_response(403, "FORBIDDEN", "Forbidden")
